...
const DialogViewBox = ({ dialogViewDetails, dialogView, oncloseViewDialog }) => {
  ...
  return (
    <div>
      <Dialog style={{ minWidth: '500px !important' }} open={open} onClose={handleClose} aria-labelledby='alert-dialog-title' aria-describedby='alert-dialog-description'>
        <DialogTitle style={{ display: 'flex', justifyContent: 'center' }} id='alert-dialog-title'>
          {details.title}
        </DialogTitle>
        <div style={{ borderBottom: '1px solid #000' }} />
        <DialogContent>
          <DialogContentText id='alert-dialog-description'>{details.comment}</DialogContentText>
        </DialogContent>
        <DialogActions>
